 ...
 */
bool QGList::insert(int index, ElemType e)
{
	if (ListIsFull())
		return false;
...
bool QGList::erase(int index)
{
	...
	if (index > List.size)
	{
		cout << "没有这么多节点，无法删除,只有" << List.size << "个节点" << endl;
		getchar();
		return false;
	}
	if (index == 1)
...
bool QGList::isLoop()
{
	Node* pnew, * pold = new Node;
	pnew = pold = List.head;
	while (pold && pold->next && pold != pnew)
	{
		pnew = pnew->next;
		pold = pold->next->next;
	}
 ...
 * 返回中间节点
 */
QGList::ElemType& QGList::middleElem()
{
	Node* pnew, * pold = new Node;
	pnew = pold = List.head;
